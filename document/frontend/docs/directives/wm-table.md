...
It is a cell contains a plain text followed by a label
...
variable =
  text: 'plain_text_or_key'
  translate: true # if the text need not to be translated, then set this value to null or undefined
  label:
    text: 'translation_key' # the text in label will be translated anyway
    type: 'primary' # the type can be 'plain', 'primary', 'info', 'warning', 'danger'. Each of them has different style
```
